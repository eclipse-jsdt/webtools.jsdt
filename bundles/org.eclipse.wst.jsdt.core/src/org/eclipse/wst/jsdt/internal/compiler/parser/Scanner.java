...
private int internalScanIdentifierOrKeyword(int index, int length, char[] data) {
	...
	switch (data[index]) {
		...
		case 'c' : // case char catch class const continue
			switch (length) {
				case 4 :
					if (data[++index] == 'a')
						if ((data[++index] == 's')
				...
				case 5 :
					if (data[++index] == 'a')
						...
						if (data[index] == 'l')
							...
							&& (data[++index] == 's')
							&& (data[++index] == 't'))
							// there is also TokenNameconst, but that is not handled
							// correctly (const is reserved in Java but unused)
							return TokenNamevar;
						else
							return TokenNameIdentifier;
				case 8 :
		...
		case 'd' : // do double delete default
			switch (length) {
				case 2 :
					if ((data[++index] == 'o'))
						return TokenNamedo;
		...
		case 'e' : // else enum export extends
			switch (length) {
				case 4 :
					if ((data[++index] == 'l')
						&& (data[++index] == 's')
		...
		case 'f' : // for final float false finally function
			switch (length) {
				case 3 :
		...
		case 'i' : // if in int import interface implements instanceof
			switch (length) {
				case 2 :
					if (data[++index] == 'f')
						return TokenNameif;
		...
		case 'p' : // public package private protected
			switch (length) {
				case 6 :
					if ((data[++index] == 'u')
						&& (data[++index] == 'b')
		...
		case 's' : // short super static switch synchronized
			switch (length) {
				case 5 :
					if (data[++index] == 'h')
						if ((data[++index] == 'o')
		...
		case 't' : // try this true throw throws typeof transient
			switch (length) {
				case 3 :
					if ((data[++index] == 'r')
						&& (data[++index] == 'y'))
